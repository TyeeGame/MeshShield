...
function card(n, online) {
  ...
  const graphColor=n.state==='QUARANTINED'?'--danger':(n.state==='ANOMALY'||n.state==='CONTAINMENT PENDING')?'--warning':'--success';
  ctx.strokeStyle=getComputedStyle(document.documentElement).getPropertyValue(graphColor).trim()||'#5be0b3';ctx.lineWidth=2;ctx.beginPath();history.forEach((v,i)=>{const x=i/Math.max(1,history.length-1)*520,y=85-v/max*70;i?ctx.lineTo(x,y):ctx.moveTo(x,y);});ctx.stroke();
