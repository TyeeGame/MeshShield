...
For the new sender/partner-inbox experience, see [the send-message demo guide](MESSAGE-DEMO.md). It requires the updated firmware and an explicit LAN startup option; the original demo below remains available.
...
Device OS **1.5.2** is retained; it is not an Argon hardware restriction. Particle cloud compilation succeeded for this target; the binary is `build/argon.bin`. The user completed the one-board USB demo checks described in [current validation](VALIDATION.md).
...
Canonical headers are in `shared/`; the sole firmware project is `firmware/argon`. The plain dashboard needs no frontend build. [Protocol](PROTOCOL.md) describes the 18-byte packet and separate JSON commands. Simulator and fake-serial tests are not hardware validation.
